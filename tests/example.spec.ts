...
test('1. Successful Redirection to Sign Up Page', async ({ page }) => {
  ...
  expect(currentUrl).toBe('http://localhost:3000/signup');  // Use expect to check the URL
});

////////// Successful Sign In //////////

test('2. Successful Login', async ({ page }) => {   // Valid Email and Password Should Sign In Successfully
    ...
    expect(currentUrl).toBe(`http://localhost:3000/main?message=Signed${space}in${space}successfully`);
});
...
test('3a. Wrong Password', async ({ page }) => {
  ...
  expect(currentUrl).toBe(`http://localhost:3000/login?message=Invalid${space}login${space}credentials`);
...
//   expect(currentUrl).toBe(`http://localhost:3000/main?message=Account%20created!`);
...
test('5. Existing User Signs Up', async ({ page }) => {
  ...
  expect(currentUrl).toBe(`http://localhost:3000/signup?message=User${space}already${space}registered`);
...
test('6. Invalid Email Format', async ({ page }) => {
  ...
  expect(currentUrl).toBe(`http://localhost:3000/signup?message=Unable${space}to${space}validate${space}email${space}address:${space}invalid${space}format`);
...
test('7. Invalid Password Format', async ({ page }) => {
  ...
  expect(currentUrl).toBe(`http://localhost:3000/signup?message=Password${space}should${space}be${space}at${space}least${space}6${space}characters.`);
